...
List<PurchaseOrder> getPurchaseOrder();
        ...
        @Query("UPDATE PurchaseOrder SET quantity = :qty WHERE productID = :id AND productVariants=:variants")
        int updateProductCardQuantity(int qty,int id,String variants);

        @Query("UPDATE PurchaseOrder SET quantity = quantity+:qty WHERE productID = :id AND productVariants=:variants")
        int updateProductQuantity(int qty,int id,String variants);
//
//        @Query("SELECT SUM(quantity) FROM PurchaseOrder")
//        int getTotalQuantity();

        @Query("SELECT COUNT(*) FROM PurchaseOrder where productID= :id AND productVariants=:variants")
